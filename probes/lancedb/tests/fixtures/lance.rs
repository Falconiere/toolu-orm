...
impl LanceDependencyUnavailable {
...
pub fn load_lance(connection: &Connection, path: &Path) -> Result<(), LanceDependencyUnavailable> {
  let engine: String = connection
    ...
    .map_err(|error| LanceDependencyUnavailable::new(format!("DuckDB version: {error}")))?;
  if engine != "v1.5.5" {
    return Err(LanceDependencyUnavailable::new(format!(
  ...
  let literal = quoted_path(path)
    .map_err(|error| LanceDependencyUnavailable::new(format!("lance extension path: {error}")))?;
  connection
    .execute(&format!("LOAD {literal}"), [])
    .map_err(|error| LanceDependencyUnavailable::new(format!("lance extension load: {error}")))?;
  ...
  let (version, loaded): (String, bool) = connection
    ...
    .map_err(|error| {
      LanceDependencyUnavailable::new(format!("lance extension metadata: {error}"))
    })?;
