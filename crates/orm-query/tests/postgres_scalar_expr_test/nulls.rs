//! AC-3 / AC-10: `COALESCE` as a projection and as an `ORDER BY` term.
...
#[tokio::test]
async fn coalesce_falls_back_to_created_at_when_last_accessed_is_null() -> TestResult {
  let client = db::setup_db("scalar_expr_coalesce").await?;
...
#[tokio::test]
async fn ordering_by_coalesce_ranks_rows_by_their_effective_time() -> TestResult {
  let client = db::setup_db("scalar_expr_coalesce_order").await?;
