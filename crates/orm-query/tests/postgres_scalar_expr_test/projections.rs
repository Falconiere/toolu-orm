//! AC-7 / AC-10: `CASE` and `||` as selected outputs on Postgres.
...
async fn case_and_concatenation_return_their_branch_values() -> TestResult {
  let client = db::setup_db("scalar_expr_case").await?;
  // Postgres resolves overloads by exact type and a bound `Value::Integer`
  // arrives as `bigint`, which `substr(text, int, int)` does not accept — so
  // the positions are literals here where the SQLite suite binds them.
  let prefix = Scalar::func(
    "substr",
    vec![Scalar::col(&BODY), Scalar::sql("1"), Scalar::sql("3")],
  )?;
...
async fn a_case_without_else_yields_null_when_no_branch_matches() -> TestResult {
  let client = db::setup_db("scalar_expr_case_null").await?;
