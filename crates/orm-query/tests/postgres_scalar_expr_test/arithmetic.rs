//! AC-4 / AC-5 / AC-10: an arithmetic self-update, and binds in SET and WHERE
//! of one statement then in SELECT, WHERE and ORDER BY of the next.
...
async fn an_arithmetic_self_update_raises_only_the_filtered_row() -> TestResult {
  let client = db::setup_db("scalar_expr_increment").await?;
  let bump = || async {
  ...
  let after_one: Vec<Memory> = all_memories().order_by(ID.asc()).fetch_all(&client).await?;
  assert_eq!(
    after_one
      .iter()
  ...
  bump().await?;
  let after_two: Vec<Memory> = all_memories()
    .filter(ID.eq("m1"))
    .fetch_all(&client)
    .await?;
...
async fn binds_land_in_select_set_and_where_in_statement_order() -> TestResult {
  let client = db::setup_db("scalar_expr_bind_order").await?;
