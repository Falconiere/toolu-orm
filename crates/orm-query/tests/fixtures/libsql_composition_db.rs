//! In-memory libsql database for the query-composition scenarios, seeded
...
pub async fn setup_db() -> Result<libsql::Connection, Box<dyn std::error::Error>> {
  ...
  for owner in OWNERS_SEED {
    ...
    match owner {
      Some(id) => insert.set(&OWNER_ID, id).execute(&conn).await?,
      None => insert.set_null(&OWNER_ID).execute(&conn).await?,
    };
  }
  for (id, owner_id) in ITEMS_SEED {
    let insert = InsertBuilder::new("items").set(&ITEM_ID, id);
    match owner_id {
      Some(owner) => insert.set(&ITEM_OWNER_ID, owner).execute(&conn).await?,
      None => insert.set_null(&ITEM_OWNER_ID).execute(&conn).await?,
    };
  }
