...
//! against an in-memory libsql database (libsql-only lane, async).
//!
//! The same five scenarios run on rusqlite and Postgres; the rendered SQL is
//! pinned separately in `join_alias_sql_test`.

#[path = "fixtures/libsql_join_db.rs"]
pub mod db;
