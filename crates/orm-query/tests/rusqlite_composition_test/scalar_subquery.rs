...
#[test]
fn a_scalar_subquery_counts_each_items_owner_rows() -> Outcome {
  let conn = setup_db()?;

  let rows: Vec<OwnerCountRow> = items_with_owner_counts().fetch_all(&conn)?;
...
#[test]
fn the_same_projection_survives_a_bounded_first_row_fetch() -> Outcome {
  let conn = setup_db()?;

  let row: OwnerCountRow = items_with_owner_counts().fetch_one(&conn)?;
