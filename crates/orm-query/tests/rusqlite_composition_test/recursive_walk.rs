...
#[test]
fn the_walk_terminates_on_a_cycle_and_stops_at_the_depth_limit() -> Outcome {
  let conn = setup_db()?;

  let rows: Vec<WalkRow> = walk_rows("b1", 2).fetch_all(&conn)?;
...
#[test]
fn a_larger_limit_reaches_the_node_one_hop_further() -> Outcome {
  let conn = setup_db()?;

  let rows: Vec<WalkRow> = walk_rows("b1", 3).fetch_all(&conn)?;
...
#[test]
fn a_zero_limit_returns_only_the_seed() -> Outcome {
  let conn = setup_db()?;

  let rows: Vec<WalkRow> = walk_rows("b1", 0).fetch_all(&conn)?;
...
/// generous the limit.
#[test]
fn a_seed_with_no_outgoing_edge_returns_only_itself() -> Outcome {
  let conn = setup_db()?;

  let rows: Vec<WalkRow> = walk_rows("b2", 9).fetch_all(&conn)?;
...
#[test]
fn a_node_reachable_by_two_paths_reports_its_shallowest_depth() -> Outcome {
  let conn = setup_db()?;

  let rows: Vec<WalkRow> = walk_rows("b1", 5).fetch_all(&conn)?;
...
#[test]
fn counting_the_walk_reports_the_number_of_reached_nodes() -> Outcome {
  let conn = setup_db()?;

  assert_eq!(walk_rows("b1", 2).count(&conn)?, 3);
  assert_eq!(walk_rows("b1", 3).count(&conn)?, 4);
  assert!(walk_rows("b1", 0).exists(&conn)?);
  Ok(())
}
