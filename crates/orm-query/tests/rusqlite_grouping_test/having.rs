...
#[test]
fn having_filters_groups_by_their_aggregate() -> TestResult {
...
#[test]
fn the_having_bound_is_a_parameter_not_a_literal() -> TestResult {
  let conn = db::setup_db()?;
...
#[test]
fn a_where_bind_and_a_having_bind_keep_their_own_values() -> TestResult {
  let conn = db::setup_db()?;
...
#[test]
fn several_having_conjuncts_all_apply() -> TestResult {
  let conn = db::setup_db()?;
...
#[test]
fn a_having_that_excludes_every_group_returns_no_rows() -> TestResult {
  let conn = db::setup_db()?;
...
#[test]
fn grouping_an_empty_filter_result_returns_no_rows() -> TestResult {
  let conn = db::setup_db()?;
...
/// `SUM` NULL, `COUNT(*)` zero.
#[test]
fn an_ungrouped_aggregate_over_no_rows_returns_one_null_row() -> TestResult {
  let conn = db::setup_db()?;
