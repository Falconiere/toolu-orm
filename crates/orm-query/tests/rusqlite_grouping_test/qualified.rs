...
#[test]
fn grouping_by_a_joined_aliased_column_counts_per_label() -> TestResult {
...
#[test]
fn an_aggregate_can_read_a_column_of_the_aliased_relation() -> TestResult {
  let conn = db::setup_db()?;
  let f = files();
  let s = sources();
...
#[test]
fn the_raw_column_expr_escape_hatch_still_projects() -> TestResult {
  let conn = db::setup_db()?;
...
#[test]
fn a_raw_grouping_term_groups_by_a_computed_key() -> TestResult {
  let conn = db::setup_db()?;
