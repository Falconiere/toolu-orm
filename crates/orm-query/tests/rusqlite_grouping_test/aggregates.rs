...
#[test]
fn every_aggregate_is_computed_per_group() -> TestResult {
  let conn = db::setup_db()?;

  let rows: Vec<Totals> = totals().fetch_all(&conn)?;
...
#[test]
fn count_distinct_differs_from_count_star_where_duplicates_exist() -> TestResult {
  let conn = db::setup_db()?;
...
#[test]
fn aggregates_compose_with_arithmetic_in_a_projection() -> TestResult {
  let conn = db::setup_db()?;
