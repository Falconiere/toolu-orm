...
async fn only_symbol(conn: &libsql::Connection) -> Result<CodeSymbol, toolu_orm_query::QueryError> {
...
#[tokio::test]
async fn a_database_clock_expression_writes_an_iso_timestamp() -> TestResult {
  let conn = db::setup_db().await?;

  let written: GeneratedId = index_with_clock("src/lib.rs")?.fetch_one(&conn).await?;
  assert_eq!(written.id, 1);

  let stamp = only_symbol(&conn).await?.indexed_at;
  assert_eq!(
    stamp.len(),
    24,
...
#[tokio::test]
async fn the_conflict_branch_recomputes_the_clock_from_its_own_binds() -> TestResult {
  let conn = db::setup_db().await?;
  ...
  let again: GeneratedId = index_with_clock("src/lib.rs")?.fetch_one(&conn).await?;
