...
#[test]
fn the_first_insert_writes_the_values_rather_than_the_update() -> TestResult {
  let conn = db::setup_db()?;
  assert_eq!(bump("t1").execute(&conn)?, 1);
...
#[test]
fn a_conflict_increments_the_counter_and_leaves_unnamed_columns_alone() -> TestResult {
  let conn = db::setup_db()?;
  seed_memory(&conn)?;
...
#[test]
fn do_nothing_reports_no_affected_row_and_changes_nothing() -> TestResult {
  let conn = db::setup_db()?;
  seed_memory(&conn)?;
  let before = memory(&conn, "m1")?;
  ...
  assert_eq!(memory(&conn, "m1")?, before);
  Ok(())
}

#[test]
fn do_nothing_still_inserts_a_row_that_does_not_conflict() -> TestResult {
  let conn = db::setup_db()?;
  seed_memory(&conn)?;
  ...
  assert_eq!(memory(&conn, "m2")?.body, "fresh");
  Ok(())
}

#[test]
fn coalesce_keeps_a_stored_workspace_and_adopts_a_missing_one() -> TestResult {
  let conn = db::setup_db()?;
  seed_memory(&conn)?;
  ...
  let upsert = |workspace: Option<&str>, expr: Scalar| {
    let builder = InsertBuilder::new("memories")
      .set(&MEMORY_ID, "m1")
      .set(&BODY, "incoming");
    let builder = match workspace {
      Some(value) => builder.set(&WORKSPACE_ID, value),
      None => builder.set_null(&WORKSPACE_ID),
    };
    builder.on_conflict(
      OnConflict::column(&MEMORY_ID)
        .set_excluded(&BODY)
        .set_scalar(&WORKSPACE_ID, expr),
    )
  };

  upsert(None, keep_stored).execute(&conn)?;
  let kept = memory(&conn, "m1")?;
  ...
  upsert(Some("w2"), adopt).execute(&conn)?;
  assert_eq!(memory(&conn, "m1")?.workspace_id.as_deref(), Some("w2"));
  Ok(())
}
