...
/// repo's rows survive.
#[tokio::test]
async fn the_delete_removes_only_the_matching_rows() -> Outcome {
  let conn = setup_db("composition_dml_1").await?;

  prune("r1", "src/a.rs").execute(&conn).await?;
  let rows: Vec<VecRow> = remaining_vec_rows().fetch_all(&conn).await?;
...
fn the_statement_binds_only_the_subquerys_own_values() {
  let (sql, params) = prune("r1", "src/a.rs").to_sql_for(Dialect::Postgres);
...
#[tokio::test]
async fn a_subquery_matching_nothing_deletes_nothing() -> Outcome {
  let conn = setup_db("composition_dml_2").await?;

  prune("r9", "nowhere.rs").execute(&conn).await?;
  let rows: Vec<VecRow> = remaining_vec_rows().fetch_all(&conn).await?;
...
#[tokio::test]
async fn the_predicate_accepts_a_compound_subquery() -> Outcome {
  let conn = setup_db("composition_dml_3").await?;
