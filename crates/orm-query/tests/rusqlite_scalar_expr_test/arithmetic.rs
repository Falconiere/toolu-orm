...
#[test]
fn an_arithmetic_self_update_raises_only_the_filtered_row() -> TestResult {
  ...
  assert_eq!(bump()?, 1);
  let after_one: Vec<Memory> = all_memories().order_by(ID.asc()).fetch_all(&conn)?;
  assert_eq!(
    after_one
      .iter()
  ...
  bump()?;
  let after_two: Vec<Memory> = all_memories().filter(ID.eq("m1")).fetch_all(&conn)?;
  assert_eq!(after_two.first().map(|row| row.access_count), Some(2));
  Ok(())
}

#[test]
fn binds_land_in_select_set_and_where_in_statement_order() -> TestResult {
  let conn = db::setup_db()?;
