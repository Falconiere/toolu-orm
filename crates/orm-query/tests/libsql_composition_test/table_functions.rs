...
/// SQL text.
#[tokio::test]
async fn json_each_expands_a_bound_json_array_into_rows() -> Outcome {
...
#[tokio::test]
async fn an_empty_json_array_expands_to_no_rows() -> Outcome {
  let conn = setup_db().await?;
  let seeds = json_each("[]")?;
...
#[tokio::test]
async fn a_function_source_joins_a_real_table() -> Outcome {
  let conn = setup_db().await?;
...
#[tokio::test]
async fn pragma_table_info_reads_a_bound_table_name() -> Outcome {
  let conn = setup_db().await?;
...
/// is still there.
#[tokio::test]
async fn a_hostile_alias_is_quoted_not_executed() -> Outcome {
  let conn = setup_db().await?;
...
#[tokio::test]
async fn the_production_json_each_seeded_walk_terminates_on_the_cycle() -> Outcome {
  let conn = setup_db().await?;

  let rows: Vec<crate::db::WalkRow> = crate::json_walk::json_seeded_walk(r#"["a"]"#, 2)?
    .fetch_all(&conn)
    .await?;
