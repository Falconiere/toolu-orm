//! Relational query execution (`with_many`/`with_one`) against a real libsql
//! connection: correlated JSON subqueries via `RelationalQuery::to_sql_sqlite()`,
//! decoded with `parse_many_column`/`parse_one_column` and
...
async fn insert_user(conn: &libsql::Connection, id: &str, name: &str) -> TestResult {
...
async fn seed(conn: &libsql::Connection) -> TestResult {
  insert_user(conn, "u1", "Alice").await?;
  insert_user(conn, "u2", "Bob").await?;
  insert_post(conn, "p1", Some("u1"), "First").await?;
  insert_post(conn, "p2", Some("u1"), "Second").await?;
  insert_post(conn, "p3", None, "Orphan").await?;
  Ok(())
}
...
#[tokio::test]
async fn with_many_reports_correct_post_counts_via_parse_many_column() -> TestResult {
...
#[tokio::test]
async fn with_many_builds_structs_via_from_relational_values() -> TestResult {
  let conn = db::setup_db().await?;
  seed(&conn).await?;
...
#[tokio::test]
async fn with_one_returns_none_for_orphan_post_and_some_for_authored_post() -> TestResult {
  let conn = db::setup_db().await?;
  seed(&conn).await?;
