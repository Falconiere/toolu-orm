//! Relational loads over a real `BLOB` column on a real in-memory rusqlite
//! database: a declared binary column round-trips byte-exactly for has-many and
//! has-one, empty and NULL stay distinct, and an undeclared BLOB still fails at
//! the database — the regression this suite guards.

#[path = "fixtures/rusqlite_blob_db.rs"]
pub mod db;
...
#[test]
fn with_many_round_trips_nonempty_empty_and_null_blobs() -> TestResult {
  ...
  let direct: Vec<u8> = conn.query_row("SELECT payload FROM files WHERE id = 'f1'", [], |r| {
    r.get(0)
  })?;
...
#[test]
fn with_one_round_trips_a_binary_column_and_reports_an_orphan() -> TestResult {
  let conn = db::setup_db()?;
...
#[test]
fn with_one_reports_a_null_binary_column_on_a_matched_parent() -> TestResult {
  let conn = db::setup_db()?;
  ...
  conn.execute("UPDATE files SET owner_id = 'o2' WHERE id = 'f3'", ())?;
  let owner_json: Option<String> = conn.query_row(&sql, [], |row| row.get(1))?;
  let text = owner_json.ok_or("o2 must match")?;
...
#[test]
fn an_undeclared_blob_column_is_still_rejected_by_sqlite() -> TestResult {
  let conn = db::setup_db()?;
