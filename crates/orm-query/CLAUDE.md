...
## Crate Type
- Library
- Internal deps: toolu-orm-core; toolu-orm-connection; sqlite-vec register helper under `sqlite-vec`
- No default driver. Features: `libsql` (async), `rusqlite` (sync), `postgres` (async), and `lancedb`; legacy `Executor` execution requires exactly one implemented driver on this crate with `lancedb` absent
...
- Legacy `execute()` methods are feature-gated: async for libsql/Postgres, sync for rusqlite. `InsertBuilder`, `UpdateBuilder`, and `DeleteBuilder` always expose async `execute_on(&impl DbConnection) -> Result<u64, DbError>`, which renders through `DbConnection::dialect()` and forwards binding/execution to the connection.
...
- No production Lance `Executor` exists in this crate. `execute_on` has no capability guards: issue #174 owns refusal of Lance `ON CONFLICT` and ordinary DML `RETURNING` before execution. `execute_on` does not fetch `RETURNING` rows.
...
- `portable_execute.rs` — unconditional async shared-connection write execution
