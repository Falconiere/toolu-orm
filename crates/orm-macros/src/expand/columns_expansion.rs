...
pub fn expand_columns_module(input: &TableInput) -> TokenStream {
  ...
  let mod_doc = format!("Column constants for the \"{table_name}\" table, generated by #[table].");
  let table_doc = format!("Name of the \"{table_name}\" table.");
  let all_columns_doc = format!("All column names of the \"{table_name}\" table.");
  ...
  let col_consts: Vec<TokenStream> = input
    ...
    .map(|col| {
      ...
      let col_doc = format!("Column \"{col_name}\" of the \"{table_name}\" table.");
      quote! {
        #[doc = #col_doc]
