...
pub fn expand(attrs: &Fts5Attrs, input: &TableInput) -> TokenStream {
  ...
  let column_calls = input.columns.iter().map(column_call);
  ...
  quote! {
    impl toolu_orm_core::table::TableSchema for #struct_name {
      fn table_def() -> toolu_orm_core::table::TableDef {
        toolu_orm_core::fts5::Fts5Table::new(#table_name)
          #(#column_calls)*
          #(#option_calls)*
          .build()
      }
...
fn column_call(column: &ColumnInput) -> TokenStream {
  ...
  quote! { .#method(#name, toolu_orm_core::column::ColumnType::Text) }
}
