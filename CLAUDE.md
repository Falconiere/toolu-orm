...
- orm-core supplies the shared schema and query types. The facade depends on core, macros, query and connection; orm-cli depends on core and connection. orm-query also depends on connection without a default driver and forwards its selected driver features. The standalone sqlite-vec register helper has no orm-core dependency, and the facade-consumer depends only on the facade.
...
## Driver features
- Features `libsql`, `rusqlite`, `postgres`, and `lancedb` exist on the facade, core, macros, query, connection, CLI and facade-consumer; the dependents forward them to orm-core. `lancedb` provides `LanceConnection::open(path)` for bundled DuckDB plus a caller-supplied pinned Lance extension, then `attach(existing_directory, namespace)` for local table lifecycle and persisted rows. `LanceDbConnection::from_namespace` consumes the attachment and implements async `DbConnection` plus runtime-free `DbConnectionBlocking` over one serialized DuckDB connection. `to_duckdb_params` binds NULL, integer, real, text, blob, and boolean `Value` variants for prepared SQL and returns a typed error for deferred tagged codecs. A manually implemented `FromRow::from_lance_row` reads named portable `LanceRow` values using `get_typed` for i64/f64/String/bool/Vec<u8>/Option<T>, with checked wide integers and column/type errors; derive support remains in #161. Portable SELECT execution and schema rendering remain separate; write builders execute through `execute_on`. The sqlite-vec register helper has no driver features.
...
- orm-query uses `cfg_single_backend!` for legacy executors and their `execute`/fetch builder methods only when exactly one implemented driver is active and `lancedb` is absent. `InsertBuilder`, `UpdateBuilder`, and `DeleteBuilder` always provide async `execute_on(&impl DbConnection) -> Result<u64, DbError>`; it renders through the connection's runtime dialect and forwards bound values to that connection. Filter building remains available across feature combinations; libsql transactions have the same single-driver gate.
