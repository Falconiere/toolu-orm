...
## Workspace
- Virtual workspace; seven crates under `crates/`: orm-core, orm-macros, orm-query, orm-connection, orm-cli, orm (the `toolu-orm` facade), and orm-facade-consumer. An eighth, `toolu-orm-sqlite-vec-register` (nested in `crates/orm-connection/`), owns the one `unsafe` call that registers sqlite-vec; it is published because orm-connection and orm-query depend on it behind their `sqlite-vec` feature, and `cargo publish` rejects any path-only dependency, optional or not.
