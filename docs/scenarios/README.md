...
| [LanceDB namespace lifecycle](lancedb-namespace-lifecycle.md) | Production attach selects a real local catalog for unqualified SQL; create/open/list/drop preserve rows across reopen and return named duplicate, missing, and validation errors. |
| [LanceDB portable scalar binding](lancedb-scalar-binding.md) | Production conversion binds six portable scalar types to prepared DuckDB statements on real Lance tables and rejects deferred tagged codecs before execution. |
...
| [LanceDB scalar result decoding](lancedb-scalar-decoding.md) | Typed fields and column-level conversion errors on real Lance rows. |

- [Shared write execution](portable-writes.md) — connection-selected INSERT, UPDATE, DELETE.
