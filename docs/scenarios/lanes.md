...
| rusqlite-only | `cargo nextest run -p toolu-orm-query --features rusqlite` and `-p toolu-orm-connection --features rusqlite,sqlite-vec` | orm-query's sync rusqlite executor, the rusqlite `DbConnection` / `DbConnectionBlocking` twin (see [Blocking connection](blocking-connection.md)), and live sqlite-vec vec0 DDL + KNN |
