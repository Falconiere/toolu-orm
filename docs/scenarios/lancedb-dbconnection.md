...
`execute_sql` and `query_map` convert all bound `Value` parameters before executing prepared SQL. Tagged timestamp, JSON, UUID, and numeric values fail with `LanceUnsupportedValue` inside `DbError::Query` before a write. `query_map` passes a `LanceRow` of named portable values to a manually implemented `FromRow::from_lance_row`; the [scalar decoder](lancedb-scalar-decoding.md) supports integer, real, text, boolean, binary and optional fields. Missing or ambiguous column names, unsupported result types, and a row type without a Lance decoder return `DbError::RowMapping`. Derive support belongs to #161.
