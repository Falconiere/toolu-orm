...
- **Only the async path queues.** The async methods now admit one operation at a time before `spawn_blocking`, so contending async callers wait on their own tasks instead of on tokio's blocking pool. Blocking callers are deliberately not gated — they take the connection mutex directly, which is why they still work while async callers are queued. See [rusqlite async backpressure](rusqlite-async-backpressure.md).
