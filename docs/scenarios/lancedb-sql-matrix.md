...
The probe renders the existing builders with `Dialect::Sqlite` and binds the returned `Value` sequence to a prepared DuckDB statement. The fixture uses `items(id BIGINT, group_id BIGINT, name VARCHAR, score BIGINT)` with `(1,1,one,10)`, `(2,1,two,20)`, `(3,2,three,30)`, `(4,99,orphan,40)`, plus `groups(1,alpha)` and `groups(2,beta)`. Each test creates independent disposable Lance datasets. Only the fixture's integer and text `Value` variants are converted in this probe; issue #148 owns the production scalar codec.

## Observed SELECT forms

The rows below passed on macOS 26.6.2 arm64 with DuckDB v1.5.5 and Lance extension `2f167ea`. `?N` is the builder's SQLite rendering and the values column follows parameter order. The parity links name existing real SQLite and PostgreSQL suites that run in the full repository gate. They prove the same SQL forms on those suites' own fixtures; same-seed three-backend acceptance belongs to #179. No syntax change was observed for these SELECT forms; issue #155 must add the session-selected Lance renderer and issues #149/#169–#172 execute the individual surfaces.
